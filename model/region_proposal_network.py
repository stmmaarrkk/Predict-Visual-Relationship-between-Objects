...
class RegionProposalNetwork(nn.Module):
    ...
    def forward(self, x, img_size, scale=1.):
        ...
        * :math:`H` and :math:`W` are height and width of the input feature.
        * :math:`A` is number of anchors assigned to each pixel.
        ...
        rois = list()
        roi_indices = list()
        for i in range(n):  # Batch is supported here ?
            roi = self.proposal_layer(
                rpn_locs[i].cpu().data.numpy(),
                rpn_fg_scores[i].cpu().data.numpy(),
                anchor, img_size,
                scale=scale)  # roi is (R,4), proposal boxes
...
class ProposalCreator:  # RPN Forwarding
    ...
    def __call__(self, loc, score,
        ...
        #print("roi {0} \nroi_through_loc {1} \n roi_clip {2} \n loc {3}\nanchor {4}".format(roi[0:5], roi_thourgh_loc[0:5], roi_clip[0:5], loc[0:5], anchor[0:5]))
        roi = roi[keep, :]
        score = score[keep]
        ...
        roi = roi[order, :]
        #print("roi shape(before nms) {0}".format(roi.shape))
